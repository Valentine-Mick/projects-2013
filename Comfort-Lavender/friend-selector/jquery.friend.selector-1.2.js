 ...
 */
 var returnArray;
 $.ajax({ url: 'https://www.thenicheguys.com/clients/comfort/lavender/database/recieveLove.php',
         type: 'post',
         success: function(output) {	
	         		output = output.replace('[','');
	         		output = output.replace(']','');
	         	
	         		var len = (output.split('"').length - 1);
	         		for(var ii = 0; ii < len; ii ++)
	         		{output = output.replace('"', '')}
	         		
	         		var array = output.split(",");
	         		array = $.map(array, function(n){
		         		return parseInt(n);
	         		});
	         		returnArray = array;
...
(function(window, document, $, undefined) {
  ...
  defaults = {
    ...
    color: "default",
    lang: {
      title: "Show your love with Lavender",
      buttonSubmit: "Send",
      buttonCancel: "Cancel",
      ...
      selectedLimitResult: "You can only send love to 1 person at a time",
...
jQuery(document).ready(function($) {
    $(".bt-fs-dialog").fSelector({
      onSubmit: function(response){
        ...
        postToFriend(selected_friends[0]);
      }
    });
  });
